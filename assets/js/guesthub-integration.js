...
function initGuestHubSession() {
    ...
    if (token && room && name) {
        ...
        if (lang) {
            ...
            // Try different language function names
            if (typeof changerLangueCustom === 'function') {
                changerLangueCustom(lang);
            } else if (typeof changerLangue === 'function') {
                changerLangue(lang);
        ...
        const roomInput = document.getElementById('guestRoomInput');
        if (roomInput) {
        ...
        setTimeout(() => {
            if (typeof verifierNomClient === 'function') {
